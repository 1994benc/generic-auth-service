...
func (handler *Handler) SetupRoutes() {
	...
	// User routes
	handler.Router.HandleFunc("/api/users", middleware.AuthMiddleware(handler.GetUsers)).Methods(http.MethodGet)
...
func (h *Handler) VerifyToken(w http.ResponseWriter, r *http.Request) {
	// Verifying that the caller has access to the ability to verify a token by checking their CONSUMER_SERVER_SECRET
	...
	consumer_server_secret := authHeaderParts[1]
	envs, err := godotenv.Read(".env")
	if err != nil {
		log.Fatal("Error loading .env file")
	}
	if consumer_server_secret != envs["CONSUMER_SERVER_SECRET"] {
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return
	}
	query := r.URL.Query()
